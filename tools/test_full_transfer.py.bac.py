        ...
        with open(source_path, 'rb') as src:
            with open(target_path, 'wb') as dst:
                while True:
                    ...
                    if int(progress) % 10 == 0 and bytes_copied > 0:
                        logger.info(f"  Прогресс: {progress:.1f}% ({bytes_copied}/{total_size} bytes)")
    ...
    try:
        ...
        def progress_callback(bytes_amount):
            ...
            if int(progress) % 10 == 0 and bytes_copied[0] > 0:
                logger.info(f"  Прогресс S3: {progress:.1f}% ({bytes_copied[0]}/{total_size} bytes)")
